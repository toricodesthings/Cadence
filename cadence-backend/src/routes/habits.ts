...
import { insertHabitSchema, updateHabitSchema, resolveHabitActionSchema, weeklyHabitsQuerySchema, monthlyHabitsQuerySchema } from "../types/habit";
...
export const habitsRoutes = new Hono<{ Bindings: Env; Variables: AuthVariables }>()
    .post("/", zValidator("json", insertHabitSchema), async (c) => {
    ...
    .get("/weekly", zValidator("query", weeklyHabitsQuerySchema), async (c) => {
        const userId = c.get("userId");
    ...
    .get("/:id/monthly", zValidator("param", uuidParamSchema), zValidator("query", monthlyHabitsQuerySchema), async (c) => {
        const userId = c.get("userId");
        const { id } = c.req.valid("param");
    ...
    .patch("/:id", zValidator("param", uuidParamSchema), zValidator("json", updateHabitSchema), async (c) => {
        const userId = c.get("userId");
        const { id } = c.req.valid("param");
        const body = c.req.valid("json");
        const db = getDbClient(c.env);
    ...
    .post("/:id/resolve", zValidator("param", uuidParamSchema), zValidator("json", resolveHabitActionSchema), async (c) => {
        const userId = c.get("userId");
        const { id } = c.req.valid("param");
