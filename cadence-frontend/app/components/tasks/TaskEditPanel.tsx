...
function formatDateTime(iso: string) {
    // Append local-midnight suffix for date-only strings to avoid UTC off-by-one
...
export function TaskEditPanel({ taskId, onClose }: TaskEditPanelProps) {
    ...
    const deadlineLabel = (() => {
        if (!task) return "No deadline";
        if (task.scheduledEnd && task.dueDate) {
            const start = formatDateTime(task.dueDate);
            const end = formatDateTime(task.scheduledEnd);
            return `${start} – ${end}`;
        }
        if (task.dueDate) return formatDateTime(task.dueDate);
        return "No deadline";
    })();
                            ...
                            >
                                <div className="flex flex-col max-h-[45vh] overflow-y-auto scrollbar-thin">
                                            ...
                                            >
                                                <div className="flex flex-col gap-2 pl-[42px] pr-5 py-3">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        >
                                                            <button className="text-[13px] text-twilight-text-soft hover:text-twilight-text transition-colors">
                                    ...
                                    <MetaRow icon={CalendarOff} label="Not before">
                                        ...
                                        >
                                            <button
                                                className="text-[13px] text-twilight-text-soft hover:text-twilight-text transition-colors"
                                            >
                                    ...
                                    {/* Deadline */}
                                    <MetaRow icon={task.scheduledEnd ? CalendarRange : Calendar} label="Deadline">
                                        <DeadlinePickerPopover
                                        ...
                                        >
                                            <button
                                                aria-label={`Deadline: ${deadlineLabel}. Click to change`}
                                                className="text-[13px] text-twilight-text-soft hover:text-twilight-text transition-colors"
                                            >
                                                {deadlineLabel}
                                            </button>
                                    ...
                                    <MetaRow icon={Bell} label="Reminder">
                                        <span className="text-[13px] text-twilight-text-muted/90">
                                            {task.reminderAt
                                                ? new Date(task.reminderAt).toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })
                                                : "None"}
                                        </span>
                                    </MetaRow>
                                    ...
                                    <MetaRow icon={Tag} label="Tags">
                                        <div className="flex flex-wrap items-center gap-1.5 flex-1 justify-end">
                                            ...
                                            <DropdownMenu.Root>
                                                <DropdownMenu.Trigger asChild>
                                                    <button className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[12px] font-medium text-twilight-text-muted hover:text-twilight-text hover:bg-white/[0.06] transition-colors cursor-pointer border border-twilight-border border-dashed">
                                                        <Plus size={12} />
                                                        Add tag
                                                    </button>
                                                </DropdownMenu.Trigger>
