...
export function TaskMenuItems({ task, onAddSubtask, MenuComponents: Menu }: TaskMenuItemsProps) {
    ...
    const handleQuickSchedule = (daysToAdd: number, startOfWeekend = false, nextWeek = false) => {
        ...
        updateTask.mutate({
            id: task.id,
            scheduledStart: date.toISOString().split("T")[0] + "T00:00:00Z",
            isAllDay: true
        });
    };
        ...
        <>
            ...
            <Menu.Sub>
                ...
                <Menu.Portal>
                    <Menu.SubContent className="w-48">
                        ...
                        <Menu.Separator />
                        <Menu.Item
                            onClick={() => updateTask.mutate({ id: task.id, scheduledStart: null, dueDate: null, isAllDay: true })}
            ...
            */}
            ...
            <TagPickerSubmenu
                ...
                activeTagIds={tags.map((t: any) => t.id)}
    ...
    return (
        <DropdownMenu.Root>
            <DropdownMenu.Trigger asChild>
                <button className="flex h-7 w-7 cursor-pointer items-center justify-center rounded-lg text-twilight-text-muted transition-colors hover:bg-white/[0.05] hover:text-twilight-text-soft">
