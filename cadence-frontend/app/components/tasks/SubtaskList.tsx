...
function SortableSubtaskItem({ subtask, onToggle, onDelete, onTitleChange }: SortableSubtaskItemProps) {
    ...
    return (
        <motion.div
            ...
            className={`group flex items-center gap-2 py-1.5 ${isDragging ? "opacity-50" : "opacity-100"}`}
        >
            <div
                {...attributes}
                {...listeners}
                className="opacity-0 group-hover:opacity-100 transition-opacity cursor-grab hover:bg-white/[0.04] p-0.5 rounded text-twilight-text-muted/50 hover:text-twilight-text-muted shrink-0"
            >
            ...
            <button
                ...
                className={`w-4 h-4 rounded-[4px] border shrink-0 flex items-center justify-center transition-colors ${subtask.isComplete
                    ? "bg-feedback-success border-feedback-success text-twilight-base"
                    : "border-twilight-border hover:border-feedback-success/50"
                    }`}
            >
                {subtask.isComplete && (
                    <svg viewBox="0 0 14 14" fill="none" className="w-[10px] h-[10px]">
            ...
            <input
                ...
                className={`flex-1 min-w-0 bg-transparent outline-none text-[13px] transition-all duration-300 ${subtask.isComplete
            ...
            <button
                ...
                className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-red-500/10 text-twilight-text-muted/50 hover:text-red-400 shrink-0"
...
export function SubtaskList({ taskId }: { taskId: string }) {
    ...
    return (
        <div className="mt-4 border-t border-twilight-border/40 pt-4">
            <span className="text-[11px] font-display font-medium text-twilight-text-muted/50 uppercase tracking-wider pl-1 font-semibold flex items-center justify-between">
                Subtasks
            </span>
            ...
            <div className="mt-2">
                ...
                <div className="flex items-center gap-2 py-1.5 pl-6 mt-1">
                    <input
                        type="text"
                        value={newTitle}
                        ...
                        className="w-full bg-transparent outline-none text-[13px] text-twilight-text-muted/90 italic placeholder:text-twilight-text-muted/80"
                    />
                </div>
            </div>
        </div>
