...
- Deployed web builds call Neon Auth through the worker proxy at `/api/auth/*` (first-party cookies, required for mobile Safari/iOS). The worker also completes the OAuth verifier exchange on `/auth/callback`.
