...
- Shell: `app/root.tsx` (HTML shell, fonts, error boundary) · `app/providers.tsx` (account-keyed query persistence + `NeonAuthUIProvider`, persistent background, `WorkspaceStartup` keeps the mounted first route hidden until its active queries and `StartupSuspense` content are ready; later navigation loads normally) · `worker.ts` (SPA static assets via `not_found_handling: "single-page-application"`, not a rendering layer).
