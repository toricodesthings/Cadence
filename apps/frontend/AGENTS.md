...
- Dark surfaces use the `panel`/`panel-raised` colors (the background's own bases; theme presets derive them from their gradient), never accent mixes; right-rail panels share the `surface-shell` class. Reuse existing utilities (`.glass`, `.glass-surface`, `.aurora-accent`, `.glow-*`, `.focus-pulse-soft`, `.cadence-toast`, `.offline-banner`, `.mobile-sheet-*`, `.safe-*`, `.scrollbar-*`). Put standard `backdrop-filter` after its WebKit fallback so production optimization preserves it. Z-index: only `.layer-*` utilities, never arbitrary `z-[…]`; popover/menu content is already `layer-floating-ui` (above dialogs), so never re-layer it.
