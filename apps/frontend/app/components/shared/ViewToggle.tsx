...
export function ViewToggle({ view, onViewChange, compact = false }: ViewToggleProps) {
    ...
    // Inside a controls menu the toggle spans the menu, one half per option.
    ...
    return (
        <div className={`${compact ? "w-full" : ""} flex items-center rounded-2xl border border-twilight-border/30 bg-twilight-base/40 p-1 backdrop-blur-md ${isCompact ? "gap-0.5" : ""}`}>
            <button
                ...
                className={`${fill} touch-target flex min-h-10 items-center gap-1.5 rounded-xl ${isCompact ? "px-3 text-[13px]" : "px-4 text-sm"} font-medium transition-all cursor-pointer
                    ${view === "list"
