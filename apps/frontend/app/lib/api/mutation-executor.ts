...
async function executeMutationOp(client: ApiClient, op: MutationOp): Promise<unknown> {
    switch (op.type) {
        ...
        case "process_inbox_to_task": {
            const { inboxItemId, rawText, title, scheduledDate, dueDate, scheduledStart, scheduledEnd, isAllDay, projectId, tagIds, priority, effort, durationEstimate, recurrenceRule, waitingOn, nlp, complete } = op.payload;
