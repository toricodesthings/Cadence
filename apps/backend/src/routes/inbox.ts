...
import { insertInboxItemSchema, updateInboxItemSchema, insertInboxSectionSchema, updateInboxSectionSchema, processInboxItemSchema } from "../types/inbox";
import { uuidParamSchema } from "../types/common";
import type { Env } from "../types/env";
import type { AuthVariables } from "../lib/auth";
import { throwIfNotFound } from "../lib/errors";
import { apiValidator } from "../lib/validation";
import { normalizeTaskTemporalFields } from "../lib/task-normalization";
import { validateTaskRecurrenceRule } from "../lib/task-recurrence";
import { normalizeSettings } from "./settings";
import { sourceSurfaceSchema } from "../types/task";
...
export const inboxRoutes = new Hono<{ Bindings: Env; Variables: AuthVariables }>()
    ...
    .post("/:id/process", apiValidator("param", uuidParamSchema), apiValidator("json", processInboxItemSchema), async (c) => {
        const userId = c.get("userId");
        const { id } = c.req.valid("param");
        const { clientMutationId, title, keepNote, scheduledDate, projectId, tagIds, priority, durationEstimate, recurrenceRule, waitingOn, nlp } = c.req.valid("json");
        const db = getDbClient(c.env);
        ...
        const result = await withRls(db, userId, async (tx) => {
            ...
            if (clientMutationId) {
                await recordMutation(tx, userId, clientMutationId, task.id);
            }
