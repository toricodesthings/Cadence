...
proxyRoutes.get("/weather", apiValidator("query", weatherQuerySchema), async (c) => {
    ...
    if (!res.ok) {
        return c.json({ error: { code: "UPSTREAM_ERROR", message: "Weather service unavailable" } }, 502);
    }
...
proxyRoutes.get("/geocode/reverse", apiValidator("query", reverseGeocodeQuerySchema), async (c) => {
    ...
    if (!res.ok) {
        return c.json({ error: { code: "UPSTREAM_ERROR", message: "Geocoding service unavailable" } }, 502);
    }
