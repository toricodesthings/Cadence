...
- **Cadence guide:** `help/*.md` (one file per topic) served by the read-only `get_cadence_help` tool (`tools/help.ts`, topic enum). Links are in-app only: routes, `?settings=<tab>`, or ids from tool results. A user-visible change that makes a help file or the primer (`blocks/base/cadence-primer.md`) wrong edits it in the same change; `tests/unit/ai-help.test.ts` checks every link against the known routes and Settings tabs.
- `tools/` — one file per callable surface: `tasks`, `projects`, `sections`, `tags`, `habits`, `inbox`, `calendar`, `metrics`, `suggestions`, `help`, `projections`. Tools read/write real user data through the same RLS-scoped path as routes — never bypass `withRls`.
