...
- `agent.ts` — model/agent construction (`getAgentInstance`, `getModelId`, reasoning effort `REASONING_EFFORT`); stamps each assistant message's metadata with `promptHash` (prompt blocks + tool names/descriptions/schemas). History passes through `dropUnsignedReasoning` (`persistence/message-mapper.ts`) before replay, so unsigned reasoning summaries never reach the provider.
- **Prompts are files, git is the only source:** system-prompt blocks are `prompt/blocks/{base,context,style}/*.md` (order and layer in `prompt/prompt-blocks.ts`), the title prompt is `title/title-prompt.md`. Edit and push; the deploy ships them. `.md` imports as text (wrangler `rules` + the vitest `md-text` plugin). An unknown `{{placeholder}}` throws in `composePrompt` — the composer tests catch it.
