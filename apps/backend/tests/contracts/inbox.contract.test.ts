...
const { getDbClientMock, withRlsMock, checkIdempotencyMock, recordMutationMock, getIdempotencyKeyMock } = vi.hoisted(() => ({
...
function createInboxApp() {
    return createTestApp("/inbox", inboxRoutes);
}
...
function createInboxProcessTx(capture: {
    ...
    return {
        select: vi.fn()
            ...
            .mockImplementationOnce(() => ({
                from: vi.fn(() => ({
                    where: vi.fn().mockResolvedValue([{ id: "tag-1", userId: TEST_USER_ID }]),
                })),
            }))
            .mockImplementationOnce(() => ({
                from: vi.fn(() => ({
...
describe("inbox route contracts", () => {
    ...
    it("supports idempotent inbox item creation via Idempotency-Key header", async () => {
        // First call: no existing mutation
        checkIdempotencyMock.mockResolvedValue(null);
        getIdempotencyKeyMock.mockReturnValue("mut-1");
        const capture: { values?: Record<string, unknown> } = {};
        const tx = createInsertTx(ITEM_ROW, capture);
        getDbClientMock.mockReturnValue(tx);
        withRlsMock.mockImplementation(async (_db: any, _userId: any, cb: any) => cb(tx));
        ...
        const response = await app.request("http://localhost/inbox", {
            method: "POST",
            headers: { "content-type": "application/json", "Idempotency-Key": "mut-1" },
            body: JSON.stringify({ rawText: "Buy groceries" }),
        });
    ...
    it("returns existing item when Idempotency-Key already processed", async () => {
    ...
    it("supports idempotent section creation via Idempotency-Key header", async () => {
        checkIdempotencyMock.mockResolvedValue(null);
        getIdempotencyKeyMock.mockReturnValue("mut-s1");
        const capture: { values?: Record<string, unknown> } = {};
        const tx = createInsertTx(SECTION_ROW, capture);
        getDbClientMock.mockReturnValue(tx);
        withRlsMock.mockImplementation(async (_db: any, _userId: any, cb: any) => cb(tx));
        ...
        const response = await app.request("http://localhost/inbox/sections", {
            method: "POST",
            headers: { "content-type": "application/json", "Idempotency-Key": "mut-s1" },
            body: JSON.stringify({ name: "Later" }),
        });
